...
- (void)viewDidLoad
{
    ...
    if(player != nil){
        ...
        self.weight.text = [NSString stringWithFormat:@"%d",player.weight];
        self.size.text = [NSString stringWithFormat:@"%d",player.size];
        
        if (player.urlProfilePic != nil) {
            NSURL *url = [NSURL URLWithString:player.urlProfilePic];
